    ...
    <div className="min-h-screen flex flex-col">
      ...
      <header className="border-b border-[var(--border)] px-6 py-4 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-4">
          <h1 className="text-2xl font-bold">🎤 Unconference</h1>
          <span
          ...
          >
            {phase === "submission" ? "📝 Propuestas abiertas" : "🗳️ Votación abierta"}
          </span>
        </div>
        ...
        <div className="flex items-center gap-3">
          {phase === "submission" && (
            <>
              <button
                ...
                className="px-4 py-2 rounded-lg bg-[var(--accent)] hover:bg-[var(--accent-hover)]
              ...
              <button
                ...
                className="px-4 py-2 rounded-lg bg-[var(--bg-card)] border border-[var(--border)]
                  hover:bg-[var(--bg-card-hover)] font-medium transition-colors cursor-pointer"
              >
                Pedir charla 🙋
              </button>
            </>
          ...
          {user?.is_admin && (
            <button
              ...
              className="px-4 py-2 rounded-lg bg-[var(--orange)]/20 text-[var(--orange)]
                hover:bg-[var(--orange)]/30 font-medium transition-colors cursor-pointer"
            >
              {phase === "submission" ? "Iniciar votación 🗳️" : "Volver a propuestas 📝"}
            </button>
          )}

          <div className="flex items-center gap-2 ml-2 pl-4 border-l border-[var(--border)]">
            <span className="text-[var(--text-muted)] text-sm">{user?.name}</span>
            <button
              onClick={handleLogout}
              className="text-[var(--text-muted)] hover:text-[var(--red)] text-sm transition-colors cursor-pointer"
            >
              Salir
            </button>
          </div>
      ...
      {/* Content */}
      {phase === "submission" ? (
        <div className="flex-1 grid grid-cols-2 divide-x divide-[var(--border)] overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-4xl mx-auto p-6 space-y-3">
            <div className="text-center mb-8">
              <h2 className="text-2xl font-bold mb-1">¡Vota por las sesiones! 🔥</h2>
